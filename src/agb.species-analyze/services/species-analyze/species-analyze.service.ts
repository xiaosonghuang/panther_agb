...
@Injectable()
export class speciesAnalyzeService {
  apiUrl: String;
  private solutionTree: speciesTreeNode;

  constructor(private http: Http, private login: LoginService) {
    this.apiUrl = `${AppSettings.API_ENDPOINT}/speciess-analyze`;
    this.solutionTree = {
      name: "All",
  ...
  /**
   * Calls agbitics api and return solution for species analyze
  ...
  createSolutionTree(parent, data: { [pathways: string]: Array<string> }) {
    ...
    let nmostActiveNode: speciesTreeNode = {
      name: mostActivePathway,
      type: speciesTreeNodeType.Pathway,
      highlight: false,
      active: true,
      children: []
    };
    ...
    for (let s of directSolution)
      nmostActiveNode.children.push(<speciesTreeNode>{
        name: s,
        active: true,
        highlight: false,
        type: speciesTreeNodeType.Solution
      });
   ...
   */
  save(key: string) {
    let apiUrl = `${AppSettings.API_ENDPOINT}/species-analyze-storage/save`;
